...
void main() {
	...
	UINT8 gameover = 0;
	UINT8 wait = 0;
	UINT8 projectile_x = 0;
	UINT8 projectile_y = 0;
	...
	while(gameover == 0) {
		...
		//Projectile-movement
		if(randomeffectvalue == 1 && paused == 0) {
			set_sprite_tile(5,4);
			projectile_x += 6;
		...
		if(paused == 0) {
			...
			//projectile
			move_sprite(5,projectile_x,projectile_y);
		...
		//NPC_2
		if(player_x == npc2_x && player_y == npc2_y) {
			gameover = 1;
		}
		
		//NPC_3
		if(player_x == npc3_x && player_y == npc3_x) {
			gameover = 1;
		}
